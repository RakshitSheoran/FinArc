...
import { getCategoryTrends } from '../../utils/computeChartData'
    ...
    <div className="flex flex-wrap items-center justify-center gap-4 mt-1">
      {payload.map((p) => (
        <div key={p.value} className="flex items-center gap-1.5">
          <span className="inline-block w-5 h-[2px] rounded-full" style={{ background: p.color }} />
          <span className="text-[11px] font-semibold text-[var(--text-secondary)]">{p.value}</span>
        </div>
      ))}
    </div>
  )
...
export default function CategoryTrendChart() {
  ...
  const data = getCategoryTrends(transactions)

  return (
    <div className="card flex flex-col gap-1">
      <div className="mb-4">
        <h2 className="text-[15px] font-bold text-[var(--text-primary)] mb-[3px]">Category Trends</h2>
      ...
      {transactions.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={data} margin={{ top: 5, right: 4, left: -10, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 11, fontFamily: 'Plus Jakarta Sans' }} axisLine={false} tickLine={false} tickFormatter={(v) => `₹${v}`} />
            <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'var(--border)', strokeWidth: 1 }} />
            <Legend content={<CustomLegend />} />
            {lines.map(({ key, color }) => (
              <Line key={key} type="monotone" dataKey={key} name={key} stroke={color} strokeWidth={2.5} dot={false} activeDot={{ r: 4, fill: color, strokeWidth: 0 }} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      )}
