...
import { getWeeklySpend } from '../../utils/computeChartData'
  ...
  function CustomTooltip({ active, payload, label }) {
    ...
    return (
      <div className="chart-tooltip">
        ...
        <p className="text-[13px] font-bold" style={{ color: isPeak ? BAR_PEAK : 'var(--accent)' }}>
    ...
    <div className="card flex flex-col gap-4">
        ...
        {maxDay.amount > 0 && (
          <div className="text-right">
            ...
            <p className="text-[13px] font-bold text-[#F59E0B]">
              {maxDay.day} — ₹{maxDay.amount.toLocaleString()}
            </p>
          </div>
        )}
      </div>
      {transactions.length === 0 ? (
        <div className="h-[200px] flex items-center justify-center text-sm text-[var(--text-muted)]">
          Add transactions to see weekly spending
        </div>
      ) : (
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data} margin={{ top: 5, right: 4, left: -10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
            <XAxis dataKey="day" tick={{ fill: 'var(--text-muted)', fontSize: 11, fontFamily: 'Plus Jakarta Sans' }} axisLine={false} tickLine={false} dy={8} />
            ...
            <Bar dataKey="amount" radius={[6, 6, 0, 0]} maxBarSize={40}>
              {data.map((entry) => (
                <Cell key={entry.day} fill={entry.day === maxDay.day && entry.amount > 0 ? BAR_PEAK : BAR_DEFAULT} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
